...
"WWWDDDDDDDDDDDCDDDDDWWWW"
"WWDDDDWWDDDDDDDDDDDDDWWW"
...
void Sandbox2D::OnAttach()
{
	...
	m_TextureStairs = Hazel::SubTexture2D::CreateFromCoords(m_SpriteSheet, { 5, 11 }, { 128, 128 });
	m_TextureTree = Hazel::SubTexture2D::CreateFromCoords(m_SpriteSheet, { 2, 1 }, { 128, 128 });
	m_TexureBarrel = Hazel::SubTexture2D::CreateFromCoords(m_SpriteSheet, { 2, 1 }, { 128, 128 }, { 1, 2 });
...
void Sandbox2D::OnUpdate(Hazel::Timestep ts)
{
	...
	if (Hazel::Input::IsMouseButtonPressed(HZ_MOUSE_BUTTON_LEFT)) {
		HZ_PROFILE_SCOPE("Emit Particles");
		...
		constexpr auto num_particles_to_emit = 5;
		for (int i = 0; i < num_particles_to_emit; i++) {
			m_ParticleSystem.Emit(m_Particle);
		}
	}
	{
		HZ_PROFILE_SCOPE("Update Particles");
		m_ParticleSystem.OnUpdate(ts);
	}
	{
		HZ_PROFILE_SCOPE("Render Particles");
		m_ParticleSystem.OnRender(m_CameraController.GetCamera());
	}

	{
		// sprite sheet
		HZ_PROFILE_SCOPE("Game Scene Render");
		Hazel::Renderer2D::BeginScene(m_CameraController.GetCamera());
		...
		//Hazel::Renderer2D::DrawQuad({ 1.0f, 0.0f, 0.5f }, { 1.0f, 1.0f }, m_TexureBarrel);
